    ...
    <div class="container-fluid mt-4">
                ...
                @if($mesesDisponibles->isNotEmpty())
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            ...
                            @foreach($mesesDisponibles as $mes)
                                <li>
                                    <a class="dropdown-item"
                                        href="{{ route('comercial.estudiantesNoActivos', ['from' => $mes->mes . '-01', 'to' => date('Y-m-t', strtotime($mes->mes . '-01'))]) }}">
                                        {{ $mes->mes_nombre }}
                            ...
                            <li>
                                <a class="dropdown-item text-danger" href="{{ route('comercial.estudiantesNoActivos') }}">
        ...
        <div class="card-soft p-3">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <form class="d-flex gap-2 flex-wrap" method="GET" action="{{ route('comercial.estudiantesNoActivos') }}">
                    ...
                    @if($from || $to || $programa || $sucursal || $search)
                        <a class="btn btn-outline-secondary" href="{{ route('comercial.estudiantesNoActivos') }}">
                ...
                <div class="btn-group gap-2">
                    <a href="{{ route('comercial.estudiantesNoActivos.exportar', request()->all()) }}" class="btn btn-success btn-sm">
                    ...
                    <tbody>
                        @forelse($estudiantesInactivos as $index => $estudiante)
                            <tr>
                                ...
                                <td>
                                    @if($estudiante->celular_tutor && $estudiante->celular_tutor != 'Sin celular')
                                        <a href="https://wa.me/{{ preg_replace('/\D/', '', $estudiante->celular_tutor) }}"
                                            target="_blank" class="text-decoration-none text-success fw-bold">
                                            <i class="bi bi-whatsapp"></i> {{ $estudiante->celular_tutor }}
                                        </a>
                                        <br>
                                        <small class="text-muted">Tutor: {{ $estudiante->nombre_tutor }}</small>
                                    @elseif($estudiante->celular_estudiante)
                                        <a href="https://wa.me/{{ preg_replace('/\D/', '', $estudiante->celular_estudiante) }}" target="_blank" class="text-decoration-none text-primary fw-bold">
                                            <i class="bi bi-person-fill"></i> {{ $estudiante->celular_estudiante }}
                                        </a>
                                        <br>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">
                                    ...
                                    @if($from || $to || $programa || $sucursal || $search)
                                        <a href="{{ route('comercial.estudiantesNoActivos') }}" class="btn btn-sm btn-primary mt-2">
