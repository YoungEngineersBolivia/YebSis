...
<div class="container-fluid mt-4">
    ...
    <div class="chart-wrap card-soft mb-4">
        <canvas id="chartInactivos" style="height: 240px"></canvas>
    </div>
    ...
    <div class="card-soft p-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="toolbar">
                ...
                <form class="d-flex gap-2" method="GET" action="{{ route('estudiantesNoActivos') }}">
        ...
        <div class="table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                @forelse($estudiantesInactivos as $estudiante)
                    <tr>
                        <td class="fw-semibold">{{ $estudiante->nombre }}</td>
                        <td>{{ $estudiante->apellido }}</td>
                        <td>{{ $estudiante->fecha_inactivacion_fmt }}</td>
                        <td class="text-center">
                            <form action="{{ route('estudiantes.reactivar', $estudiante->id) }}" method="POST"
                                  onsubmit="return confirm('¿Reactivar a este estudiante?')">
                                @csrf
                                @method('PUT')
                ...
                @empty
                    <tr><td colspan="4" class="text-center text-muted">Sin estudiantes inactivos.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
