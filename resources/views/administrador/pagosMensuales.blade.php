...
@section('content')
    <div class="page-header d-flex flex-wrap justify-content-between align-items-center gap-3">
        ...
        <div class="d-flex align-items-center gap-3">
            <form action="{{ route('pagos.mensuales') }}" method="GET" class="d-flex align-items-center gap-3">
                ...
                <a href="{{ route('pagos.mensuales') }}" class="btn btn-sm btn-outline-secondary shadow-sm"
            ...
            <div class="btn-group shadow-sm">
                <a href="{{ route('pagos.formato') }}" class="btn btn-outline-success">
            ...
            <a href="{{ route('pagos.form') }}" class="btn btn-primary shadow-sm">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover table-striped align-middle mb-0" id="tablaPagosMensuales">
                    ...
                    <tbody>
                        @forelse($ingresosMensuales as $ingreso)
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <span class="fw-bold text-capitalize">{{ $ingreso->nombre_mes }}</span>
                                            <span class="text-muted ms-1">{{ $ingreso->anio }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('pagos.index', ['mes' => $ingreso->mes, 'anio' => $ingreso->anio]) }}"
                                        class="badge bg-info text-white rounded-pill px-3 text-decoration-none hover-shadow"
                                        title="Ver detalles de estos pagos">
                                        {{ $ingreso->cantidad_pagos }} pagos <i class="bi bi-eye ms-1"></i>
                                    </a>
                                </td>
                                <td class="text-center">
                                ...
                                <td class="pe-4 text-end">
                                    <small class="text-muted">
                                        ...
                                        {{ \Carbon\Carbon::parse($ingreso->ultima_fecha)->format('d/m/Y H:i') }}
                                    </small>
                                </td>
                            </tr>
                        @empty
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="fw-bold text-primary fs-4">
                                        Bs. {{ number_format($ingresosMensuales->sum('total'), 2, '.', ',') }}
                                    </span>
                                </td>
                                <td></td>
                            </tr>
    ...
    <div class="modal fade" id="modalImportarPagos" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <form method="POST" action="{{ route('pagos.importar') }}" enctype="multipart/form-data">
                    @csrf
