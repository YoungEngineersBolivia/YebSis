            ...
            <div class="col-12 text-center">
                <div class="btn-group w-100 d-flex flex-wrap" role="group" aria-label="Tipo de acción">
                    <input type="radio" class="btn-check" name="tipoAccion" id="optionPublicacion" value="publicacion"
                        autocomplete="off" {{ old('form_type', 'publicacion') == 'publicacion' ? 'checked' : '' }}>
                    ...
                    <input type="radio" class="btn-check" name="tipoAccion" id="optionNotificacion" value="notificacion"
                        autocomplete="off" {{ old('form_type') == 'notificacion' ? 'checked' : '' }}>
                        ...
                        <form action="{{ route('publicaciones.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nombre" id="nombre" class="form-control" required
                                    placeholder="Título de la publicación" value="{{ old('nombre') }}">
                                @error('nombre') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>
                            <div class="mb-3">
                                <label for="descripcion" class="form-label fw-semibold">Descripción</label>
                                <textarea name="descripcion" id="descripcion" rows="4" class="form-control" required
                                    placeholder="Contenido de la publicación">{{ old('descripcion') }}</textarea>
                        ...
                        <form action="{{ route('notificaciones.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nombre" id="nombre_notif" class="form-control" required
                                    placeholder="Asunto de la notificación" value="{{ old('nombre') }}">
                                @error('nombre') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <textarea name="descripcion" id="descripcion_notif" rows="3" class="form-control" required
                                    placeholder="Mensaje para los tutores">{{ old('descripcion') }}</textarea>
                                @error('descripcion') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-2 mb-2">
                                    <input type="radio" class="btn-check" name="filtroDestinatarios" id="filtroTodos"
                                        value="todos" {{ old('filtroDestinatarios', 'todos') == 'todos' ? 'checked' : '' }}>
                                    ...
                                    @foreach($tutores as $tutor)
                                        ...
                                        <div class="form-check tutor-item border-bottom py-1"
                                            ...
                                            <input class="form-check-input tutor-checkbox" type="checkbox" name="tutores[]"
                                                value="{{ $tutor->Id_tutores }}" id="tutor-{{ $tutor->Id_tutores }}" {{ $isChecked ? 'checked' : '' }}>
                                            <label class="form-check-label d-flex justify-content-between w-100 pe-1"
                                                for="tutor-{{ $tutor->Id_tutores }}" style="cursor: pointer;">
                                                <span class="text-truncate">{{ $tutor->persona->Nombre ?? '' }}
                                                    {{ $tutor->persona->Apellido ?? '' }}</span>
                                                @if($esActivo)
