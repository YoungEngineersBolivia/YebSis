...
class HorariosController extends Controller
{
    public function index()
    {
        ...
        // Filtramos solo los estudiantes con rol 4
        ...
        // Filtramos solo los profesores con rol 2
        $profesores = Profesor::with('persona')
            ->whereHas('persona', function ($query) {
                $query->where('Id_roles', 2);
            })
            ->orderBy('Id_profesores')
            ->get();

        $programas = Programa::orderBy('Id_programas')->get();
    ...
    {
        $estudiantes = Estudiante::with('persona')
            ->whereHas('persona', fn($q) => $q->where('Id_roles', 4))
            ->get();

        $profesores = Profesor::with('persona')
            ->whereHas('persona', fn($q) => $q->where('Id_roles', 2))
            ->get();
